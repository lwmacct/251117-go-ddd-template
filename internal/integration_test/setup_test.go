// Package integration_test 提供端到端集成测试。
//
// 本包测试完整的业务流程，包括：
//   - 用户认证流程 (注册 → 登录 → Token 验证)
//   - RBAC 权限流程 (角色 → 权限 → 检查)
//   - 审计日志流程 (操作 → 记录 → 查询)
//
// 使用 SQLite 内存数据库进行测试，无需外部依赖。
...
func (e *TestEnv) CreateTestUser(t *testing.T, username, email, password string) *user.User {
	t.Helper()
