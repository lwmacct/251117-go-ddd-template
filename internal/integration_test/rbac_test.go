// Package integration_test 提供 RBAC 权限流程集成测试
...
func TestRBACFlow_UserRoleAssignment(t *testing.T) {
	...
	t.Run("用户角色分配流程", func(t *testing.T) {
		// 步骤 1: 创建用户
		testUser := env.CreateTestUser(t, "rbac_user", "rbac@example.com", "Password123!")
	...
	t.Run("更新用户角色（替换）", func(t *testing.T) {
		// 创建用户
		testUser := env.CreateTestUser(t, "rbac_user2", "rbac2@example.com", "Password123!")
	...
	t.Run("清空用户角色", func(t *testing.T) {
		// 创建用户
		testUser := env.CreateTestUser(t, "rbac_user3", "rbac3@example.com", "Password123!")
