...
// UserHandlerNew 用户处理器（新架构）
type UserHandlerNew struct {
...
// NewUserHandlerNew 创建用户处理器
func NewUserHandlerNew(
	createUserHandler *userCommand.CreateUserHandler,
	updateUserHandler *userCommand.UpdateUserHandler,
	deleteUserHandler *userCommand.DeleteUserHandler,
	getUserHandler *userQuery.GetUserHandler,
...
// Create 创建用户
// POST /api/users
func (h *UserHandlerNew) Create(c *gin.Context) {
...
// GetByID 获取用户详情
// GET /api/users/:id
func (h *UserHandlerNew) GetByID(c *gin.Context) {
	idStr := c.Param("id")
...
func (h *UserHandlerNew) List(c *gin.Context) {
...
// Update 更新用户
// PUT /api/users/:id
func (h *UserHandlerNew) Update(c *gin.Context) {
	idStr := c.Param("id")
	id, err := strconv.ParseUint(idStr, 10, 32)
	if err != nil {
		c.JSON(400, gin.H{"error": "invalid user id"})
...
// Delete 删除用户
// DELETE /api/users/:id
func (h *UserHandlerNew) Delete(c *gin.Context) {
	idStr := c.Param("id")
	id, err := strconv.ParseUint(idStr, 10, 32)
	if err != nil {
		c.JSON(400, gin.H{"error": "invalid user id"})
