...
type QueryRepository interface {
	...
	GetByIDWithTeams(ctx context.Context, id uint) (*Organization, error)

	// GetByIDWithMembers 根据 ID 获取组织（包含成员列表）
	GetByIDWithMembers(ctx context.Context, id uint) (*Organization, error)

	// List 获取组织列表（分页）
	List(ctx context.Context, offset, limit int) ([]*Organization, error)
	...
	Search(ctx context.Context, keyword string, offset, limit int) ([]*Organization, error)
