...
func BenchmarkUser_HasRole(b *testing.B) {
	...
	for i := range 10 {
		roles[i] = newTestRole(uint(i+1), "role"+string(rune('0'+i)))
	}
...
func BenchmarkUser_HasPermission(b *testing.B) {
	...
	for i := range 10 {
		...
		for j := range 10 {
			perms[j] = newTestPermission(uint(i*10+j+1), "perm:"+string(rune('0'+i))+":"+string(rune('0'+j)))
		}
		roles[i] = newTestRole(uint(i+1), "role"+string(rune('0'+i)), perms...)
	}
	user := &User{Roles: roles}
...
func BenchmarkUser_GetPermissions_Deduplication(b *testing.B) {
	...
	for i := range 5 {
		perms := []role.Permission{
			sharedPerm, // 共享权限
			newTestPermission(uint(i*10+2), "unique:"+string(rune('0'+i))),
		}
		roles[i] = newTestRole(uint(i+1), "role"+string(rune('0'+i)), perms...)
	}
	user := &User{Roles: roles}
