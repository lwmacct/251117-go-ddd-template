...
func TestCacheAPI(t *testing.T) {
	...
	t.Run("Info", func(t *testing.T) {
		info, err := manualtest.Get[cache.CacheInfoDTO](c, "/api/system/cache/info", nil)
	...
	t.Run("ScanAllKeys", func(t *testing.T) {
		result, err := manualtest.Get[cache.ScanKeysResultDTO](c, "/api/system/cache/keys", nil)
	...
	t.Run("ScanSettingKeys", func(t *testing.T) {
		result, err := manualtest.Get[cache.ScanKeysResultDTO](c, "/api/system/cache/keys", map[string]string{
			"pattern": "setting:*",
		})
	...
	t.Run("GetKey", func(t *testing.T) {
		// 先扫描获取一个 key（任意类型）
		scanResult, err := manualtest.Get[cache.ScanKeysResultDTO](c, "/api/system/cache/keys", map[string]string{
			"limit": "1",
		})
		require.NoError(t, err, "扫描 Keys 失败")
		...
		// 获取 key 的值（使用查询参数）
		value, err := manualtest.Get[cache.CacheValueDTO](c, "/api/system/cache/key", map[string]string{
			"key": key,
		})
	...
	t.Run("DeleteSingleKey", func(t *testing.T) {
		...
		// 删除 key（使用查询参数）
		_, err = manualtest.Delete[cache.DeleteResultDTO](c, "/api/system/cache/key", map[string]string{
			"key": key,
		})
		...
		// 验证 key 不存在
		_, err = manualtest.Get[cache.CacheValueDTO](c, "/api/system/cache/key", map[string]string{
			"key": key,
		})
	...
	t.Run("DeleteByPattern", func(t *testing.T) {
		// 先查看 schema 缓存数量
		before, err := manualtest.Get[cache.ScanKeysResultDTO](c, "/api/system/cache/keys", map[string]string{
			"pattern": "schema:*",
		})
		...
		// 按 pattern 删除
		result, err := manualtest.Delete[cache.DeleteResultDTO](c, "/api/system/cache/keys", map[string]string{
			"pattern": "schema:*",
		})
		...
		// 验证已删除
		after, err := manualtest.Get[cache.ScanKeysResultDTO](c, "/api/system/cache/keys", map[string]string{
			"pattern": "schema:*",
		})
		require.NoError(t, err, "扫描失败")
