...
func TestMenusFlow(t *testing.T) {
	...
	t.Cleanup(func() {
		if childMenuID > 0 {
			_ = c.Delete(fmt.Sprintf("/api/system/menus/%d", childMenuID))
		}
		if createdMenuID > 0 {
			_ = c.Delete(fmt.Sprintf("/api/system/menus/%d", createdMenuID))
		}
	})

	// 测试 1: 获取菜单列表
	t.Log("\n测试 1: 获取菜单列表")
	menus, _, err := helper.GetList[menu.MenuDTO](c, "/api/system/menus", nil)
	...
	created, err := helper.Post[menu.CreateResultDTO](c, "/api/system/menus", createReq)
	...
	// 测试 3: 获取菜单详情并验证字段
	t.Log("\n测试 3: 获取菜单详情并验证字段")
	detail, err := helper.Get[menu.MenuDTO](c, fmt.Sprintf("/api/system/menus/%d", created.ID), nil)
	...
	updated, err := helper.Put[menu.MenuDTO](c, fmt.Sprintf("/api/system/menus/%d", created.ID), updateReq)
	...
	childResult, err := helper.Post[menu.CreateResultDTO](c, "/api/system/menus", childReq)
	...
	t.Log("\n测试 6: 验证菜单列表数量")
	menusAfter, _, err := helper.GetList[menu.MenuDTO](c, "/api/system/menus", nil)
	require.NoError(t, err, "获取菜单列表失败")
	...
	// 测试 7: 删除子菜单
	t.Log("\n测试 7: 删除子菜单")
	err = c.Delete(fmt.Sprintf("/api/system/menus/%d", childResult.ID))
	...
	// 测试 8: 删除父菜单
	t.Log("\n测试 8: 删除父菜单")
	err = c.Delete(fmt.Sprintf("/api/system/menus/%d", created.ID))
	...
	// 测试 9: 验证删除后获取应失败
	t.Log("\n测试 9: 验证删除后获取应返回 404")
	_, err = helper.Get[menu.MenuDTO](c, fmt.Sprintf("/api/system/menus/%d", created.ID), nil)
...
func TestListMenus(t *testing.T) {
	...
	menus, meta, err := helper.GetList[menu.MenuDTO](c, "/api/system/menus", nil)
	require.NoError(t, err, "获取菜单列表失败")
...
func TestMenuReorder(t *testing.T) {
	...
	t.Cleanup(func() {
		for _, id := range []uint{menu1ID, menu2ID, menu3ID} {
			if id > 0 {
				_ = c.Delete(fmt.Sprintf("/api/system/menus/%d", id))
			}
		}
	})
	...
	for i, order := range []int{1, 2, 3} {
		...
		created, createErr := helper.Post[menu.CreateResultDTO](c, "/api/system/menus", createReq)
	...
	// 步骤 3: 验证顺序已更新
	t.Log("\n步骤 3: 验证顺序已更新")
	detail1, err := helper.Get[menu.MenuDTO](c, fmt.Sprintf("/api/system/menus/%d", menu1ID), nil)
	require.NoError(t, err, "获取菜单 %d 详情失败", menu1ID)
	detail2, err := helper.Get[menu.MenuDTO](c, fmt.Sprintf("/api/system/menus/%d", menu2ID), nil)
	require.NoError(t, err, "获取菜单 %d 详情失败", menu2ID)
	detail3, err := helper.Get[menu.MenuDTO](c, fmt.Sprintf("/api/system/menus/%d", menu3ID), nil)
