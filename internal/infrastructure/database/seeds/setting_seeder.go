...
func (s *SettingSeeder) Seed(ctx context.Context, db *gorm.DB) error {
	...
	definitions := []_persistence.SettingModel{
		// ==================== General 常规设置 ====================
		// basic 分组 - 基础信息
		{
			Key: "general.site_name", DefaultValue: datatypes.JSON(`""`), Category: "general", Group: "basic",
			ValueType: "string", Label: "站点名称", Order: 10,
			UIConfig: datatypes.JSON(`{"input_type":"text","hint":"显示在浏览器标签和页面标题中"}`),
		},
		{
			Key: "general.site_url", DefaultValue: datatypes.JSON(`""`), Category: "general", Group: "basic",
			ValueType: "string", Label: "站点 URL", Order: 20,
			UIConfig: datatypes.JSON(`{"input_type":"url","hint":"站点完整 URL，如 https://example.com"}`),
		},
		{
			Key: "general.admin_email", DefaultValue: datatypes.JSON(`""`), Category: "general", Group: "basic",
			ValueType: "string", Label: "管理员邮箱", Order: 30,
			UIConfig: datatypes.JSON(`{"input_type":"email","hint":"用于接收系统通知和报警邮件"}`),
		},
		// locale 分组 - 本地化设置
		{
			Key: "general.timezone", DefaultValue: datatypes.JSON(`"Asia/Shanghai"`), Category: "general", Group: "locale",
		...
		{
			Key: "general.language", DefaultValue: datatypes.JSON(`"zh-CN"`), Category: "general", Group: "locale",
		...
		// appearance 分组 - 外观设置
		{
			Key: "general.theme", DefaultValue: datatypes.JSON(`"light"`), Category: "general", Group: "appearance",
		...
		// password 分组 - 密码策略
		{
			Key: "security.password_min_length", DefaultValue: datatypes.JSON(`8`), Category: "security", Group: "password",
			ValueType: "number", Label: "密码最小长度", Order: 10,
			UIConfig: datatypes.JSON(`{"input_type":"number","hint":"建议至少 8 位","validation":{"min":6,"max":32}}`),
		},
		{
			Key: "security.max_login_attempts", DefaultValue: datatypes.JSON(`5`), Category: "security", Group: "password",
		...
		// session 分组 - 会话管理
		{
			Key: "security.session_timeout", DefaultValue: datatypes.JSON(`30`), Category: "security", Group: "session",
		...
		// advanced 分组 - 高级安全
		{
			Key: "security.enable_twofa", DefaultValue: datatypes.JSON(`false`), Category: "security", Group: "advanced",
		...
		// general 分组 - 通知总开关
		{
			Key: "notification.enable_notifications", DefaultValue: datatypes.JSON(`true`), Category: "notification", Group: "general",
			ValueType: "boolean", Label: "启用系统通知", Order: 10,
			UIConfig: datatypes.JSON(`{"input_type":"switch","hint":"关闭后所有通知渠道将停止发送"}`),
		},
		// email 分组 - 邮件通知
		{
			Key: "notification.enable_email", DefaultValue: datatypes.JSON(`true`), Category: "notification", Group: "email",
		...
		// sms 分组 - 短信通知
		{
			Key: "notification.enable_sms", DefaultValue: datatypes.JSON(`false`), Category: "notification", Group: "sms",
		...
		// general 分组 - 备份总开关
		{
			Key: "backup.enable_backup", DefaultValue: datatypes.JSON(`false`), Category: "backup", Group: "general",
			ValueType: "boolean", Label: "启用自动备份", Order: 10,
			UIConfig: datatypes.JSON(`{"input_type":"switch","hint":"开启数据自动备份功能"}`),
		},
		// schedule 分组 - 备份计划
		{
			Key: "backup.backup_frequency", DefaultValue: datatypes.JSON(`24`), Category: "backup", Group: "schedule",
		...
		{
			Key: "backup.retention_days", DefaultValue: datatypes.JSON(`30`), Category: "backup", Group: "schedule",
	...
	result := db.Clauses(clause.OnConflict{
		...
		DoUpdates: clause.AssignmentColumns([]string{
			"ui_config", "group", "order", "label",
		}), // 更新 UI 元数据字段，不覆盖用户修改的默认值
	}).Create(&definitions)
	if result.Error != nil {
		return result.Error
	}
