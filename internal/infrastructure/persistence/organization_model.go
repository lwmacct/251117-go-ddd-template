...
// OrganizationModel 定义组织的 GORM 持久化模型
//
...
type OrganizationModel struct {
	...
	// 聚合关系
	Teams   []TeamModel               `gorm:"foreignKey:OrganizationID"`
	Members []OrganizationMemberModel `gorm:"foreignKey:OrganizationID"`
}
...
func newOrganizationModelFromEntity(entity *organization.Organization) *OrganizationModel {
	if entity == nil {
		return nil
	}
...
func (m *OrganizationModel) ToEntity() *organization.Organization {
	if m == nil {
		return nil
	}
...
func mapOrganizationModelsToEntities(models []OrganizationModel) []*organization.Organization {
	if len(models) == 0 {
		return nil
	}

	orgs := make([]*organization.Organization, 0, len(models))
	for i := range models {
