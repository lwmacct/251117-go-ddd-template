...
func newPermissionModelFromEntity(entity *role.Permission) *PermissionModel {
	if entity == nil {
		return nil
	}
...
func (m *PermissionModel) ToEntity() *role.Permission {
	if m == nil {
		return nil
	}
...
func mapPermissionModelsToEntities(models []PermissionModel) []role.Permission {
	if len(models) == 0 {
		return nil
	}

	permissions := make([]role.Permission, 0, len(models))
	for i := range models {
...
func mapPermissionEntitiesToModels(perms []role.Permission) []PermissionModel {
	if len(perms) == 0 {
		return nil
	}
