...
type PersonalAccessTokenModel struct {
	...
	Permissions domainPat.PermissionList `gorm:"type:jsonb;not null"`
	...
	IPWhitelist domainPat.StringList `gorm:"type:jsonb"`
...
func newPATModelFromEntity(entity *domainPat.PersonalAccessToken) *PersonalAccessTokenModel {
	if entity == nil {
		return nil
	}
...
func (m *PersonalAccessTokenModel) ToEntity() *domainPat.PersonalAccessToken {
	if m == nil {
		return nil
	}

	entity := &domainPat.PersonalAccessToken{
		ID:          m.ID,
...
func mapPATModelsToEntities(models []PersonalAccessTokenModel) []*domainPat.PersonalAccessToken {
	if len(models) == 0 {
		return nil
	}

	entities := make([]*domainPat.PersonalAccessToken, 0, len(models))
	for i := range models {
