...
func NewOrganizationQueryRepository(db *gorm.DB) organization.QueryRepository {
	return &organizationQueryRepository{db: db}
}

// GetByID 根据 ID 获取组织
func (r *organizationQueryRepository) GetByID(ctx context.Context, id uint) (*organization.Organization, error) {
...
// GetByName 根据名称获取组织
func (r *organizationQueryRepository) GetByName(ctx context.Context, name string) (*organization.Organization, error) {
	var model OrganizationModel
...
func (r *organizationQueryRepository) GetByIDWithTeams(ctx context.Context, id uint) (*organization.Organization, error) {
	var model OrganizationModel
...
func (r *organizationQueryRepository) GetByIDWithMembers(ctx context.Context, id uint) (*organization.Organization, error) {
	var model OrganizationModel
...
// List 获取组织列表（分页）
func (r *organizationQueryRepository) List(ctx context.Context, offset, limit int) ([]*organization.Organization, error) {
	var models []OrganizationModel
	if err := r.db.WithContext(ctx).Offset(offset).Limit(limit).Find(&models).Error; err != nil {
		return nil, fmt.Errorf("failed to list organizations: %w", err)
	}
	return mapOrganizationModelsToEntities(models), nil
}
...
func (r *organizationQueryRepository) ExistsByName(ctx context.Context, name string) (bool, error) {
	var count int64
	if err := r.db.WithContext(ctx).Model(&OrganizationModel{}).Where("name = ?", name).Count(&count).Error; err != nil {
...
func (r *organizationQueryRepository) Search(ctx context.Context, keyword string, offset, limit int) ([]*organization.Organization, error) {
	var models []OrganizationModel
