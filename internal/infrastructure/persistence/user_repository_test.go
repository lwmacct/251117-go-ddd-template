// Package persistence 提供 User Repository 的集成测试。
//
// 使用 SQLite in-memory 数据库进行真实的数据库操作测试。
package persistence
...
func TestUserQueryRepository_ExistsByUsername(t *testing.T) {
	ctx := context.Background()
...
func TestUserQueryRepository_ExistsByEmail(t *testing.T) {
	ctx := context.Background()
