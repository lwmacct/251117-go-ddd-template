...
func NewTeamQueryRepository(db *gorm.DB) organization.TeamQueryRepository {
	return &teamQueryRepository{db: db}
}

// GetByID 根据 ID 获取团队
func (r *teamQueryRepository) GetByID(ctx context.Context, id uint) (*organization.Team, error) {
	var model TeamModel
...
func (r *teamQueryRepository) GetByOrgAndName(ctx context.Context, orgID uint, name string) (*organization.Team, error) {
	var model TeamModel
...
func (r *teamQueryRepository) GetByIDWithMembers(ctx context.Context, id uint) (*organization.Team, error) {
	var model TeamModel
...
// ListByOrg 获取组织的所有团队
func (r *teamQueryRepository) ListByOrg(ctx context.Context, orgID uint, offset, limit int) ([]*organization.Team, error) {
...
func (r *teamQueryRepository) BelongsToOrg(ctx context.Context, teamID, orgID uint) (bool, error) {
	var count int64
	if err := r.db.WithContext(ctx).Model(&TeamModel{}).
		Where("id = ? AND organization_id = ?", teamID, orgID).
		Count(&count).Error; err != nil {
