// Package command 提供 BatchCreateUsersHandler 单元测试。
package command
...
func TestBatchCreateUsersHandler_Handle(t *testing.T) {
...
func TestBatchCreateUsersHandler_PasswordPolicyNormal(t *testing.T) {
	...
	t.Run("短密码被拒绝", func(t *testing.T) {
		...
		assert.True(t, errors.Is(domainAuth.ErrWeakPassword, domainAuth.ErrWeakPassword))
	})
}
