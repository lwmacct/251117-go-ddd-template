...
func TestGetUserHandler_Handle_Success(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "获取用户基本信息",
			query: GetQuery{UserID: 1, WithRoles: false},
			user: &domainUser.User{
				ID:        1,
				Username:  "testuser",
		...
		{
			name:  "获取用户包含角色信息",
			query: GetQuery{UserID: 1, WithRoles: true},
			user: &domainUser.User{
				ID:        1,
				Username:  "admin",
...
func TestGetUserHandler_Handle_Error(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "用户不存在（不含角色）",
			query: GetQuery{UserID: 999, WithRoles: false},
		...
		{
			name:  "用户不存在（含角色）",
			query: GetQuery{UserID: 999, WithRoles: true},
			setupMocks: func(qryRepo *MockUserQueryRepository) {
...
func TestGetUserHandler_Handle_RoleConversion(t *testing.T) {
	...
	handler := NewGetHandler(mockQryRepo)
	result, err := handler.Handle(context.Background(), GetQuery{UserID: 1, WithRoles: true})
