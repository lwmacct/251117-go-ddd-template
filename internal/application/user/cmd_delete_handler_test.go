...
func TestDeleteUserHandler_Handle_Success(t *testing.T) {
	tests := []struct {
		...
		{
			name: "成功删除用户",
			cmd:  DeleteCommand{UserID: 1},
		},
		{
			name: "删除另一用户",
			cmd:  DeleteCommand{UserID: 100},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler := NewDeleteHandler(mockCmdRepo, mockQryRepo, mockEventBus)
...
func TestDeleteUserHandler_Handle_NilEventBus(t *testing.T) {
	...
	handler := NewDeleteHandler(mockCmdRepo, mockQryRepo, nil)
	err := handler.Handle(context.Background(), DeleteCommand{UserID: 1})
