...
func TestBatchCreateUsersHandler_Handle_AllSuccess(t *testing.T) {
	...
	handler := NewBatchCreateHandler(mockCmdRepo, mockQryRepo, mockAuthService)

	// Act
	result, err := handler.Handle(context.Background(), BatchCreateCommand{Users: users})
...
func TestBatchCreateUsersHandler_Handle_EmailValidationError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "用户名为空",
			users:   []BatchItemDTO{{Username: "", Email: "test@example.com", Password: "password"}},
			wantErr: "用户名不能为空",
		},
		{
			name:    "用户名只有空格",
		...
		{
			name:    "邮箱为空",
			users:   []BatchItemDTO{{Username: "validuser", Email: "", Password: "password"}},
			wantErr: "邮箱不能为空",
		},
		{
			name:    "邮箱格式无效",
			users:   []BatchItemDTO{{Username: "validuser", Email: "invalid-email", Password: "password"}},
			wantErr: "邮箱格式无效",
		},
		{
			name:    "密码为空",
			users:   []BatchItemDTO{{Username: "validuser", Email: "test@example.com", Password: ""}},
			wantErr: "密码不能为空",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := handler.Handle(context.Background(), BatchCreateCommand{Users: tt.users})
