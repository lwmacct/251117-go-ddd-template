...
func TestUpdateUserHandler_Handle_Success(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "更新用户名",
			cmd:  UpdateCommand{UserID: 1, Username: &username},
			existingUser: &user.User{
				ID:        1,
				Username:  "olduser",
		...
		{
			name: "更新多个字段",
			cmd:  UpdateCommand{UserID: 1, Email: &email, FullName: &fullName},
			existingUser: &user.User{
				ID:        1,
				Username:  "user1",
				Email:     "old@example.com",
		...
		{
			name: "更新状态为 inactive",
			cmd:  UpdateCommand{UserID: 1, Status: &status},
			existingUser: &user.User{
				ID:        1,
				Username:  "user1",
...
func TestUpdateUserHandler_Handle_Error(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "邮箱已被占用",
			cmd:  UpdateCommand{UserID: 1, Email: &email},
			setupMocks: func(cmdRepo *MockUserCommandRepository, qryRepo *MockUserQueryRepository) {
				qryRepo.On("GetByID", mock.Anything, uint(1)).Return(&user.User{
					ID:    1,
					Email: "old@example.com",
		...
		{
			name: "无效的状态值",
			cmd:  UpdateCommand{UserID: 1, Status: &invalidStatus},
			setupMocks: func(cmdRepo *MockUserCommandRepository, qryRepo *MockUserQueryRepository) {
				qryRepo.On("GetByID", mock.Anything, uint(1)).Return(&user.User{
					ID:     1,
					Status: "active",
...
func TestUpdateUserHandler_Handle_UpdateAllFields(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			cmd:  UpdateCommand{UserID: 1, Avatar: &avatar, Bio: &bio},
			existingUser: &user.User{
				ID:        1,
				Username:  "user1",
				Email:     "user@example.com",
		...
		{
			name: "更新状态为 banned",
			cmd:  UpdateCommand{UserID: 1, Status: &statusBanned},
			existingUser: &user.User{
				ID:        1,
				Username:  "user1",
				Email:     "user@example.com",
		...
		{
			name: "更新状态为 active",
			cmd:  UpdateCommand{UserID: 1, Status: &statusActive},
			existingUser: &user.User{
				ID:        1,
				Username:  "user1",
				Email:     "user@example.com",
