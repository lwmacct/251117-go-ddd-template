...
func TestListUsersHandler_Handle_Success(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "列出第一页用户",
			query: ListQuery{Page: 1, Limit: 10},
			users: []*domainUser.User{
		...
		{
			name:  "分页查询",
			query: ListQuery{Page: 2, Limit: 5},
			users: []*domainUser.User{
...
func TestListUsersHandler_Handle_Error(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "搜索数据库错误",
			query: ListQuery{Page: 1, Limit: 10, Search: "test"},
			setupMocks: func(qryRepo *MockUserQueryRepository) {
...
func TestListUsersQuery_GetOffset(t *testing.T) {
	tests := []struct {
		...
		{name: "第一页", query: ListQuery{Page: 1, Limit: 10}, offset: 0},
		{name: "第二页", query: ListQuery{Page: 2, Limit: 10}, offset: 10},
		{name: "第三页", query: ListQuery{Page: 3, Limit: 5}, offset: 10},
		{name: "页码为0时默认第一页", query: ListQuery{Page: 0, Limit: 10}, offset: 0},
	}
