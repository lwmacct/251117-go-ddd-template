// Package user 提供 mapper 函数的单元测试
package user
...
func TestToUserWithRolesResponse(t *testing.T) {
	t.Run("转换带角色的用户", func(t *testing.T) {
		...
		result := ToUserWithRolesResponse(u)
	...
	t.Run("转换nil返回nil", func(t *testing.T) {
		result := ToUserWithRolesResponse(nil)
		assert.Nil(t, result)
	})
