...
func TestCreateUserHandler_Handle_Success(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler := NewCreateHandler(mockCmdRepo, mockQryRepo, mockAuthService)
...
func TestCreateUserHandler_Handle_Error(t *testing.T) {
	tests := []struct {
		...
		{
			name: "密码策略验证失败",
			cmd:  CreateCommand{Username: "test", Email: "test@example.com", Password: "123"},
		...
		{
			name: "用户名已存在",
			cmd:  CreateCommand{Username: "existing", Email: "test@example.com", Password: "password123"},
			setupMocks: func(cmdRepo *MockUserCommandRepository, qryRepo *MockUserQueryRepository, authService *MockAuthService) {
		...
		{
			name: "邮箱已存在",
			cmd:  CreateCommand{Username: "newuser", Email: "existing@example.com", Password: "password123"},
			setupMocks: func(cmdRepo *MockUserCommandRepository, qryRepo *MockUserQueryRepository, authService *MockAuthService) {
				authService.On("ValidatePasswordPolicy", mock.Anything, "password123").Return(nil)
...
func TestCreateUserHandler_Handle_DefaultStatus(t *testing.T) {
	...
	handler := NewCreateHandler(mockCmdRepo, mockQryRepo, mockAuthService)
	_, err := handler.Handle(context.Background(), CreateCommand{
		Username: "test",
		Email:    "test@example.com",
		Password: "password123",
	})
...
func TestCreateUserHandler_Handle_CheckUsernameError(t *testing.T) {
	...
	handler := NewCreateHandler(mockCmdRepo, mockQryRepo, mockAuthService)
	result, err := handler.Handle(context.Background(), CreateCommand{
		Username: "test",
		Email:    "test@example.com",
		Password: "password123",
	})
