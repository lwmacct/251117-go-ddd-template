...
func TestGetLogHandler_Handle_Success(t *testing.T) {
	...
	handler := NewGetHandler(mockRepo)

	// Act
	result, err := handler.Handle(context.Background(), GetQuery{LogID: 1})
...
func TestGetLogHandler_Handle_NotFound(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Act
			result, err := handler.Handle(context.Background(), GetQuery{LogID: 999})
