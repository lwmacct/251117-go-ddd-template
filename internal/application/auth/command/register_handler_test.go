// Package command 提供 RegisterHandler 单元测试。
package command
...
func TestRegisterHandler_Handle(t *testing.T) {
	...
	t.Run("密码策略验证失败 - 密码太短", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, domainAuth.ErrWeakPassword, "应该返回密码太弱错误")
	...
	t.Run("用户名已存在", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, user.ErrUsernameAlreadyExists, "应该返回用户名已存在错误")
		assert.Nil(t, result)
	})
	...
	t.Run("邮箱已存在", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, user.ErrEmailAlreadyExists, "应该返回邮箱已存在错误")
		assert.Nil(t, result)
	})
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				if tt.expectedErr != nil {
					assert.ErrorIs(t, err, tt.expectedErr, "错误类型应该匹配")
				}
			} else {
