// Package command 提供 RefreshTokenHandler 单元测试
package command
...
func TestRefreshTokenHandler_Handle(t *testing.T) {
	...
	t.Run("用户不存在", func(t *testing.T) {
		...
		assert.Error(t, err)
		assert.ErrorIs(t, err, domainAuth.ErrUserNotFound)
		assert.Nil(t, result)
	})
	...
	t.Run("用户被禁用", func(t *testing.T) {
		...
		assert.Error(t, err)
		assert.ErrorIs(t, err, domainAuth.ErrUserBanned)
		assert.Nil(t, result)
	})
	...
	t.Run("用户未激活", func(t *testing.T) {
		...
		assert.Error(t, err)
		assert.ErrorIs(t, err, domainAuth.ErrUserInactive)
		assert.Nil(t, result)
	})
