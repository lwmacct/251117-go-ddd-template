...
func TestGenerateCaptchaHandler_Handle_RandomMode_Success(t *testing.T) {
	...
	handler := NewGenerateHandler(mockRepo, mockService)

	// Act
	result, err := handler.Handle(context.Background(), GenerateCommand{
		DevMode: false,
	})
...
func TestGenerateCaptchaHandler_Handle_Error(t *testing.T) {
	tests := []struct {
		...
		{
			name: "生成随机验证码失败",
			cmd:  GenerateCommand{DevMode: false},
		...
		{
			name: "生成自定义验证码图片失败",
			cmd:  GenerateCommand{DevMode: true, CustomCode: "123456"},
			setupMocks: func(repo *MockCaptchaCommandRepository, service *MockCaptchaService) {
