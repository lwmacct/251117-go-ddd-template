//nolint:dupl // 与 disable_token_handler_test.go 测试结构相似，但测试不同 Handler
package pat
...
func TestDeleteTokenHandler_Handle_Success(t *testing.T) {
	// Arrange
	...
	mockPATCmdRepo.On("Delete", mock.Anything, uint(1)).Return(nil)

	handler := NewDeleteTokenHandler(mockPATCmdRepo, mockPATQryRepo)

	// Act
	err := handler.Handle(context.Background(), DeleteTokenCommand{
		UserID:  1,
		TokenID: 1,
	})
...
func TestDeleteTokenHandler_Handle_Error(t *testing.T) {
	tests := []struct {
		name       string
		...
		{
			name: "Token 不存在",
			cmd:  DeleteTokenCommand{UserID: 1, TokenID: 999},
		...
		{
			name: "Token 不属于该用户",
			cmd:  DeleteTokenCommand{UserID: 1, TokenID: 2},
			setupMocks: func(patCmd *MockPATCommandRepository, patQry *MockPATQueryRepository) {
				token := &domainPAT.PersonalAccessToken{
					ID:     2,
					UserID: 999, // 不同的用户
		...
		{
			name: "删除失败",
			cmd:  DeleteTokenCommand{UserID: 1, TokenID: 1},
			setupMocks: func(patCmd *MockPATCommandRepository, patQry *MockPATQueryRepository) {
				token := &domainPAT.PersonalAccessToken{
					ID:     1,
					UserID: 1,
				...
				patCmd.On("Delete", mock.Anything, uint(1)).Return(errors.New("db error"))
			},
			wantErr: "failed to delete token",
		},
	}
