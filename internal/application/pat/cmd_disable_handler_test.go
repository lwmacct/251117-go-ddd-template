...
func TestDisableHandler_Handle_Success(t *testing.T) {
	...
	handler := NewDisableHandler(mockPATCmdRepo, mockPATQryRepo)

	// Act
	err := handler.Handle(context.Background(), DisableCommand{
		UserID:  1,
		TokenID: 1,
	})
...
func TestDisableHandler_Handle_Error(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Token 不存在",
			cmd:  DisableCommand{UserID: 1, TokenID: 999},
		...
		{
			name: "Token 不属于该用户",
			cmd:  DisableCommand{UserID: 1, TokenID: 2},
			setupMocks: func(patCmd *MockPATCommandRepository, patQry *MockPATQueryRepository) {
				token := &domainPAT.PersonalAccessToken{
					ID:     2,
					UserID: 999, // 不同的用户
		...
		{
			name: "禁用失败",
			cmd:  DisableCommand{UserID: 1, TokenID: 1},
			setupMocks: func(patCmd *MockPATCommandRepository, patQry *MockPATQueryRepository) {
				token := &domainPAT.PersonalAccessToken{
					ID:     1,
					UserID: 1,
