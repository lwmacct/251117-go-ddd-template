...
func TestCreateHandler_Handle_Success(t *testing.T) {
	...
	handler := NewCreateHandler(mockPATCmdRepo, mockUserQryRepo, mockTokenGen)
...
func TestCreateHandler_Handle_InheritAllPermissions(t *testing.T) {
	// Arrange
	mockPATCmdRepo := new(MockPATCommandRepository)
	mockUserQryRepo := new(MockUserQueryRepository)
	mockTokenGen := new(MockTokenGenerator)
...
func TestCreateHandler_Handle_Error(t *testing.T) {
	tests := []struct {
		name       string
		cmd        CreateCommand
		...
		{
			name: "用户不存在",
			cmd:  CreateCommand{UserID: 999, Name: "Test"},
			setupMocks: func(patCmd *MockPATCommandRepository, userQry *MockUserQueryRepository, tokenGen *MockTokenGenerator) {
		...
		{
			name: "用户没有权限",
			cmd:  CreateCommand{UserID: 1, Name: "Test"},
			setupMocks: func(patCmd *MockPATCommandRepository, userQry *MockUserQueryRepository, tokenGen *MockTokenGenerator) {
				user := &domainUser.User{
					ID:       1,
					Username: "noPermUser",
