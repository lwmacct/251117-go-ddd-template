...
func TestDeleteHandler_Handle_Success(t *testing.T) {
	...
	handler := NewDeleteHandler(mockPATCmdRepo, mockPATQryRepo)
...
func TestDeleteHandler_Handle_Error(t *testing.T) {
	tests := []struct {
		name       string
		cmd        DeleteCommand
		...
		{
			name: "Token 不存在",
			cmd:  DeleteCommand{UserID: 1, TokenID: 999},
		...
		{
			name: "Token 不属于该用户",
			cmd:  DeleteCommand{UserID: 1, TokenID: 2},
			setupMocks: func(patCmd *MockPATCommandRepository, patQry *MockPATQueryRepository) {
				token := &domainPAT.PersonalAccessToken{
					ID:     2,
					UserID: 999, // 不同的用户
		...
		{
			name: "删除失败",
			cmd:  DeleteCommand{UserID: 1, TokenID: 1},
			setupMocks: func(patCmd *MockPATCommandRepository, patQry *MockPATQueryRepository) {
				token := &domainPAT.PersonalAccessToken{
					ID:     1,
					UserID: 1,
