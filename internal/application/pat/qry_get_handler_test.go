...
func TestGetHandler_Handle_Success(t *testing.T) {
	...
	handler := NewGetHandler(mockPATQryRepo)

	// Act
	result, err := handler.Handle(context.Background(), GetQuery{
		UserID:  1,
		TokenID: 1,
	})
...
func TestGetHandler_Handle_Error(t *testing.T) {
	tests := []struct {
		name       string
		query      GetQuery
		...
		{
			name:  "Token 不存在",
			query: GetQuery{UserID: 1, TokenID: 999},
		...
		{
			name:  "Token 不属于该用户",
			query: GetQuery{UserID: 1, TokenID: 2},
			setupMocks: func(patQry *MockPATQueryRepository) {
				token := &domainPAT.PersonalAccessToken{
					ID:     2,
					UserID: 999, // 不同的用户
