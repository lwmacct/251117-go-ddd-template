...
func TestEnableHandler_Handle_Success(t *testing.T) {
	// Arrange
	...
	handler := NewEnableHandler(mockPATCmdRepo, mockPATQryRepo)

	// Act
	err := handler.Handle(context.Background(), EnableCommand{
		UserID:  1,
		TokenID: 1,
	})
...
func TestEnableHandler_Handle_Success_NoExpiration(t *testing.T) {
	// Arrange
	mockPATCmdRepo := new(MockPATCommandRepository)
	mockPATQryRepo := new(MockPATQueryRepository)
...
func TestEnableHandler_Handle_Error(t *testing.T) {
	tests := []struct {
		name       string
		cmd        EnableCommand
		...
		{
			name: "Token 不存在",
			cmd:  EnableCommand{UserID: 1, TokenID: 999},
		...
		{
			name: "Token 不属于该用户",
			cmd:  EnableCommand{UserID: 1, TokenID: 2},
			setupMocks: func(patCmd *MockPATCommandRepository, patQry *MockPATQueryRepository) {
				token := &domainPAT.PersonalAccessToken{
					ID:     2,
					UserID: 999, // 不同的用户
		...
		{
			name: "Token 已过期，无法启用",
			cmd:  EnableCommand{UserID: 1, TokenID: 1},
			setupMocks: func(patCmd *MockPATCommandRepository, patQry *MockPATQueryRepository) {
