...
func TestListHandler_Handle_Error(t *testing.T) {
	tests := []struct {
		name       string
		query      ListQuery
...
func TestListHandler_Handle_EmptyCategory(t *testing.T) {
	// 验证空分类调用 FindAll
	mockQryRepo := new(MockSettingQueryRepository)
	...
	handler := NewListHandler(mockQryRepo)
	_, err := handler.Handle(context.Background(), ListQuery{Category: ""})
...
func TestListHandler_Handle_WithCategory(t *testing.T) {
	...
	handler := NewListHandler(mockQryRepo)
	_, err := handler.Handle(context.Background(), ListQuery{Category: setting.CategorySecurity})
