...
// Handle 处理创建设置命令
func (h *CreateSettingHandler) Handle(ctx context.Context, cmd CreateSettingCommand) (*CreateSettingResult, error) {
	// 1. 验证 Key 是否已存在
	...
	if existing != nil {
		return nil, fmt.Errorf("setting key already exists")
	}
