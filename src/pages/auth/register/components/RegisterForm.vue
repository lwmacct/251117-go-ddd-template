      ...
      <v-card-text class="px-4 pb-6">
        <v-form @submit.prevent="handleRegister">
          <v-text-field
            ...
            :error-messages="
              registerStore.email.value.length > 0 && !registerStore.email.value.includes('@') ? '请输入有效的邮箱地址' : ''
            "
          ...
          <v-row dense class="mb-3">
            <v-col cols="12" sm="6">
              <v-text-field
                ...
                :error-messages="
                  registerStore.password.value.length > 0 && registerStore.password.value.length < 6 ? '密码至少6个字符' : ''
                "
              ></v-text-field>
            </v-col>
