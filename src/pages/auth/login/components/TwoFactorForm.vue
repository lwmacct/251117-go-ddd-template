...
async function handleVerify() {
  ...
  try {
    ...
    errorMessage.value = (error as Error).message || "验证失败，请检查验证码";
    twoFactorCode.value = "";
  } finally {
...
<template>
  <div class="two-factor-form-wrapper">
    <v-card class="two-factor-card" elevation="0">
      ...
      <v-card-actions class="justify-center pb-6">
        <v-btn variant="text" prepend-icon="mdi-arrow-left" @click="goBack"> 返回登录 </v-btn>
      </v-card-actions>
    </v-card>
  </div>
</template>
