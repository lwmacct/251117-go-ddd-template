...
- [目录概览](#目录概览) `:25+25`
- [核心目录详解](#核心目录详解) `:50+155`
  - [internal/ - 业务核心](#internal-业务核心) `:52+81`
  - [src/ - 前端源代码](#src-前端源代码) `:133+38`
  - [docs/ - 文档系统](#docs-文档系统) `:171+18`
  - [configs/ - 配置文件](#configs-配置文件) `:189+8`
  - [testing/ - 测试脚本](#testing-测试脚本) `:197+8`
- [文件命名规范](#文件命名规范) `:205+27`
  - [Go 文件](#go-文件) `:207+11`
  - [前端文件](#前端文件) `:218+14`
- [构建产物](#构建产物) `:232+11`
- [配置优先级](#配置优先级) `:243+7`
...
├── modules.go         # 模块结构体定义
│                      # - InfrastructureModule (DB, Redis, EventBus)
│                      # - RepositoriesModule (所有 CQRS 仓储)
│                      # - ServicesModule (领域服务 + 基础设施服务)
│                      # - HandlersModule (所有 HTTP Handler)
├── usecases.go        # UseCases 子模块定义
│                      # - AuthUseCases, UserUseCases, RoleUseCases 等
├── init_infra.go      # 基础设施初始化
├── init_repos.go      # 仓储初始化
├── init_services.go   # 服务初始化
├── init_usecases.go   # 用例初始化
├── init_handlers.go   # Handler 初始化
├── init_events.go     # 事件处理器初始化
└── init_router.go     # 路由初始化
```

初始化顺序：`Infra → Repos → Services → UseCases → Events → Handlers → Router`
