...
- [命令概览](#命令概览) `:33+10`
- [核心命令](#核心命令) `:43+77`
  - [api - 启动 HTTP 服务](#api-启动-http-服务) `:45+20`
  - [migrate - 数据库迁移](#migrate-数据库迁移) `:65+21`
  - [seed - 数据填充](#seed-数据填充) `:86+19`
  - [validate - 配置验证](#validate-配置验证) `:105+15`
- [全局选项](#全局选项) `:120+11`
- [使用 Task 运行](#使用-task-运行) `:131+15`
- [开发模式](#开发模式) `:146+27`
  - [热重载运行](#热重载运行) `:148+15`
  - [构建命令](#构建命令) `:163+10`
- [生产部署](#生产部署) `:173+29`
  - [Docker 运行](#docker-运行) `:175+13`
  - [Systemd 服务](#systemd-服务) `:188+14`
- [命令示例](#命令示例) `:202+41`
  - [完整初始化流程](#完整初始化流程) `:204+16`
  - [数据库重置](#数据库重置) `:220+10`
  - [生产环境检查](#生产环境检查) `:230+13`
- [故障排查](#故障排查) `:243+34`
  - [命令不存在](#命令不存在) `:245+8`
  - [权限错误](#权限错误) `:253+8`
  - [配置错误](#配置错误) `:261+8`
  - [数据库连接失败](#数据库连接失败) `:269+8`
