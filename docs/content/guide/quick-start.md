...
- [安装步骤](#安装步骤) `:36+124`
  - [1. 克隆项目](#1-克隆项目) `:38+7`
  - [2. 启动依赖服务](#2-启动依赖服务) `:45+13`
  - [3. 配置应用](#3-配置应用) `:58+32`
  - [4. 数据库迁移](#4-数据库迁移) `:90+24`
  - [5. 填充种子数据 (可选)](#5-填充种子数据-可选) `:114+14`
  - [6. 运行应用](#6-运行应用) `:128+32`
- [验证安装](#验证安装) `:160+51`
  - [健康检查](#健康检查) `:162+16`
  - [注册用户](#注册用户) `:178+13`
  - [登录获取 Token](#登录获取-token) `:191+13`
  - [访问受保护的端点](#访问受保护的端点) `:204+7`
- [下一步](#下一步) `:211+8`
- [故障排查](#故障排查) `:219+29`
  - [数据库连接失败](#数据库连接失败) `:221+10`
  - [Redis 连接失败](#redis-连接失败) `:231+10`
  - [端口被占用](#端口被占用) `:241+7`
