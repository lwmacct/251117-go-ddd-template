...
- [安装步骤](#安装步骤) `:34+124`
  - [1. 克隆项目](#1-克隆项目) `:36+7`
  - [2. 启动依赖服务](#2-启动依赖服务) `:43+13`
  - [3. 配置应用](#3-配置应用) `:56+32`
  - [4. 数据库迁移](#4-数据库迁移) `:88+24`
  - [5. 填充种子数据 (可选)](#5-填充种子数据-可选) `:112+14`
  - [6. 运行应用](#6-运行应用) `:126+32`
- [验证安装](#验证安装) `:158+51`
  - [健康检查](#健康检查) `:160+16`
  - [注册用户](#注册用户) `:176+13`
  - [登录获取 Token](#登录获取-token) `:189+13`
  - [访问受保护的端点](#访问受保护的端点) `:202+7`
- [下一步](#下一步) `:209+8`
- [故障排查](#故障排查) `:217+29`
  - [数据库连接失败](#数据库连接失败) `:219+10`
  - [Redis 连接失败](#redis-连接失败) `:229+10`
  - [端口被占用](#端口被占用) `:239+7`
