...
- [架构概览](#架构概览) `:31+14`
- [分层说明](#分层说明) `:45+176`
  - [1. Commands 层 (CLI 命令)](#1-commands-层-cli-命令) `:47+30`
  - [2. Adapters 层 (适配器)](#2-adapters-层-适配器) `:77+25`
  - [3. Application 层 (应用)](#3-application-层-应用) `:102+30`
  - [4. Domain 层 (领域)](#4-domain-层-领域) `:132+25`
  - [5. Infrastructure 层 (基础设施)](#5-infrastructure-层-基础设施) `:157+25`
  - [6. Bootstrap 层 (引导)](#6-bootstrap-层-引导) `:182+39`
- [数据流](#数据流) `:221+40`
  - [请求处理流程](#请求处理流程) `:223+22`
  - [依赖关系](#依赖关系) `:245+16`
- [设计模式](#设计模式) `:261+90`
  - [1. 仓储模式 (Repository Pattern)](#1-仓储模式-repository-pattern) `:263+26`
  - [2. 依赖注入 (Dependency Injection)](#2-依赖注入-dependency-injection) `:289+32`
  - [3. 适配器模式 (Adapter Pattern)](#3-适配器模式-adapter-pattern) `:321+30`
- [扩展应用](#扩展应用) `:351+22`
  - [添加新功能](#添加新功能) `:353+8`
  - [添加新的数据源](#添加新的数据源) `:361+6`
  - [添加新的外部接口](#添加新的外部接口) `:367+6`
- [最佳实践](#最佳实践) `:373+9`
- [下一步](#下一步) `:382+6`
