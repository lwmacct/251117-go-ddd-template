...
- [架构概览](#架构概览) `:31:44`
- [分层说明](#分层说明) `:45:46`
  - [1. Commands 层 (CLI 命令)](#1-commands-层-cli-命令) `:47:76`
  - [2. Adapters 层 (适配器)](#2-adapters-层-适配器) `:77:101`
  - [3. Application 层 (应用)](#3-application-层-应用) `:102:131`
  - [4. Domain 层 (领域)](#4-domain-层-领域) `:132:156`
  - [5. Infrastructure 层 (基础设施)](#5-infrastructure-层-基础设施) `:157:181`
  - [6. Bootstrap 层 (引导)](#6-bootstrap-层-引导) `:182:220`
- [数据流](#数据流) `:221:222`
  - [请求处理流程](#请求处理流程) `:223:244`
  - [依赖关系](#依赖关系) `:245:260`
- [设计模式](#设计模式) `:261:262`
  - [1. 仓储模式 (Repository Pattern)](#1-仓储模式-repository-pattern) `:263:288`
  - [2. 依赖注入 (Dependency Injection)](#2-依赖注入-dependency-injection) `:289:320`
  - [3. 适配器模式 (Adapter Pattern)](#3-适配器模式-adapter-pattern) `:321:350`
- [扩展应用](#扩展应用) `:351:352`
  - [添加新功能](#添加新功能) `:353:360`
  - [添加新的数据源](#添加新的数据源) `:361:366`
  - [添加新的外部接口](#添加新的外部接口) `:367:372`
- [最佳实践](#最佳实践) `:373:381`
- [下一步](#下一步) `:382:387`
