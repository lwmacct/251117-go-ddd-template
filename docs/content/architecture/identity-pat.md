...
- [什么是 Personal Access Token?](#什么是-personal-access-token) `:46:55`
  - [PAT vs JWT 对比](#pat-vs-jwt-对比) `:56:66`
- [Token 格式](#token-格式) `:67:78`
- [创建 Personal Access Token](#创建-personal-access-token) `:79:80`
  - [前提条件](#前提条件) `:81:85`
  - [创建 Token](#创建-token) `:86:142`
  - [权限选择](#权限选择) `:143:171`
- [使用 Personal Access Token](#使用-personal-access-token) `:172:173`
  - [基本用法](#基本用法) `:174:184`
  - [在不同场景中使用](#在不同场景中使用) `:185:186`
- [管理 Personal Access Tokens](#管理-personal-access-tokens) `:277:278`
  - [查看 Token 列表](#查看-token-列表) `:279:318`
  - [查看单个 Token 详情](#查看单个-token-详情) `:319:327`
  - [删除 Token](#删除-token) `:328:350`
- [最佳实践](#最佳实践) `:351:352`
  - [1. 最小权限原则](#1-最小权限原则) `:353:368`
  - [2. 设置合理的有效期](#2-设置合理的有效期) `:369:387`
  - [3. 使用描述性名称](#3-使用描述性名称) `:388:402`
  - [4. 配置 IP 白名单](#4-配置-ip-白名单) `:403:412`
  - [5. 定期轮换 Token](#5-定期轮换-token) `:413:418`
  - [6. 安全存储](#6-安全存储) `:419:443`
- [安全建议](#安全建议) `:444:445`
  - [Token 泄露处理](#token-泄露处理) `:446:460`
  - [监控建议](#监控建议) `:461:468`
  - [IP 白名单注意事项](#ip-白名单注意事项) `:469:474`
- [Token 状态说明](#token-状态说明) `:475:484`
- [常见问题](#常见问题) `:485:486`
  - [Q1: Token 创建后忘记保存，如何找回？](#q1-token-创建后忘记保存如何找回) `:487:493`
  - [Q2: 如何增加 Token 的权限？](#q2-如何增加-token-的权限) `:494:501`
  - [Q3: Token 过期后会自动删除吗？](#q3-token-过期后会自动删除吗) `:502:505`
  - [Q4: 使用 PAT 时出现 403 错误？](#q4-使用-pat-时出现-403-错误) `:506:522`
  - [Q5: JWT 和 PAT 可以同时使用吗？](#q5-jwt-和-pat-可以同时使用吗) `:523:535`
  - [Q6: 如何批量管理 Token？](#q6-如何批量管理-token) `:536:543`
  - [Q7: PAT 支持通配符权限吗？](#q7-pat-支持通配符权限吗) `:544:555`
- [相关文档](#相关文档) `:556:561`
- [技术实现](#技术实现) `:562:593`
