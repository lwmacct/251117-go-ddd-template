...
- [什么是 Personal Access Token?](#什么是-personal-access-token) `:46+21`
  - [PAT vs JWT 对比](#pat-vs-jwt-对比) `:56+11`
- [Token 格式](#token-格式) `:67+12`
- [创建 Personal Access Token](#创建-personal-access-token) `:79+93`
  - [前提条件](#前提条件) `:81+5`
  - [创建 Token](#创建-token) `:86+57`
  - [权限选择](#权限选择) `:143+29`
- [使用 Personal Access Token](#使用-personal-access-token) `:172+105`
  - [基本用法](#基本用法) `:174+11`
  - [在不同场景中使用](#在不同场景中使用) `:185+92`
- [管理 Personal Access Tokens](#管理-personal-access-tokens) `:277+74`
  - [查看 Token 列表](#查看-token-列表) `:279+40`
  - [查看单个 Token 详情](#查看单个-token-详情) `:319+9`
  - [删除 Token](#删除-token) `:328+23`
- [最佳实践](#最佳实践) `:351+93`
  - [1. 最小权限原则](#1-最小权限原则) `:353+16`
  - [2. 设置合理的有效期](#2-设置合理的有效期) `:369+19`
  - [3. 使用描述性名称](#3-使用描述性名称) `:388+15`
  - [4. 配置 IP 白名单](#4-配置-ip-白名单) `:403+10`
  - [5. 定期轮换 Token](#5-定期轮换-token) `:413+6`
  - [6. 安全存储](#6-安全存储) `:419+25`
- [安全建议](#安全建议) `:444+31`
  - [Token 泄露处理](#token-泄露处理) `:446+15`
  - [监控建议](#监控建议) `:461+8`
  - [IP 白名单注意事项](#ip-白名单注意事项) `:469+6`
- [Token 状态说明](#token-状态说明) `:475+10`
- [常见问题](#常见问题) `:485+71`
  - [Q1: Token 创建后忘记保存，如何找回？](#q1-token-创建后忘记保存如何找回) `:487+7`
  - [Q2: 如何增加 Token 的权限？](#q2-如何增加-token-的权限) `:494+8`
  - [Q3: Token 过期后会自动删除吗？](#q3-token-过期后会自动删除吗) `:502+4`
  - [Q4: 使用 PAT 时出现 403 错误？](#q4-使用-pat-时出现-403-错误) `:506+17`
  - [Q5: JWT 和 PAT 可以同时使用吗？](#q5-jwt-和-pat-可以同时使用吗) `:523+13`
  - [Q6: 如何批量管理 Token？](#q6-如何批量管理-token) `:536+8`
  - [Q7: PAT 支持通配符权限吗？](#q7-pat-支持通配符权限吗) `:544+12`
- [相关文档](#相关文档) `:556+6`
- [技术实现](#技术实现) `:562+32`
