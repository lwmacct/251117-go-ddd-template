...
- [概述](#概述) `:66:67`
  - [什么是 RBAC？](#什么是-rbac) `:68:80`
  - [核心概念](#核心概念) `:81:94`
- [权限模型](#权限模型) `:95:96`
  - [领域实体与持久化模型](#领域实体与持久化模型) `:97:98`
  - [权限代码格式](#权限代码格式) `:195:233`
  - [通配符权限匹配](#通配符权限匹配) `:234:266`
  - [预设角色](#预设角色) `:267:275`
- [JWT Token 与权限流转](#jwt-token-与权限流转) `:276:277`
  - [JWT Claims 结构](#jwt-claims-结构) `:278:306`
  - [登录时的权限加载流程](#登录时的权限加载流程) `:307:366`
- [Personal Access Token 认证](#personal-access-token-认证) `:367:370`
  - [PAT vs JWT 对比](#pat-vs-jwt-对比) `:371:383`
  - [PAT Token 格式](#pat-token-格式) `:384:401`
  - [统一认证中间件](#统一认证中间件) `:402:424`
  - [PAT 认证流程](#pat-认证流程) `:425:460`
- [中间件系统](#中间件系统) `:461:462`
  - [中间件执行顺序](#中间件执行顺序) `:463:471`
  - [1. Auth 中间件（统一认证）](#1-auth-中间件统一认证) `:472:507`
  - [2. RBAC 中间件（授权）](#2-rbac-中间件授权) `:508:511`
  - [3. AuditMiddleware（审计日志）](#3-auditmiddleware审计日志) `:688:726`
- [API 路由保护](#api-路由保护) `:727:728`
  - [路由配置](#路由配置) `:729:775`
  - [管理员接口保护示例](#管理员接口保护示例) `:776:799`
  - [用户接口保护示例](#用户接口保护示例) `:800:817`
- [完整的请求流程](#完整的请求流程) `:818:819`
  - [流程图](#流程图) `:820:845`
  - [成功案例：管理员创建用户](#成功案例管理员创建用户) `:846:927`
  - [失败案例：普通用户访问管理员接口](#失败案例普通用户访问管理员接口) `:928:972`
  - [所有权检查案例：用户修改自己的资料](#所有权检查案例用户修改自己的资料) `:973:1021`
- [系统初始化](#系统初始化) `:1022:1023`
  - [数据库迁移](#数据库迁移) `:1024:1046`
  - [RBAC 种子数据](#rbac-种子数据) `:1047:1091`
- [使用指南](#使用指南) `:1092:1093`
  - [1. 登录获取 Token](#1-登录获取-token) `:1094:1123`
  - [2. 为用户分配角色](#2-为用户分配角色) `:1124:1167`
  - [3. 创建自定义角色](#3-创建自定义角色) `:1168:1180`
  - [4. 为角色分配权限](#4-为角色分配权限) `:1181:1225`
  - [5. 查看审计日志](#5-查看审计日志) `:1226:1267`
- [最佳实践](#最佳实践) `:1268:1269`
  - [安全建议](#安全建议) `:1270:1297`
  - [权限设计原则](#权限设计原则) `:1298:1315`
  - [Token 管理建议](#token-管理建议) `:1316:1337`
  - [审计日志使用](#审计日志使用) `:1338:1356`
- [常见问题](#常见问题) `:1357:1358`
  - [Q1: 权限变更后何时生效？](#q1-权限变更后何时生效) `:1359:1371`
  - [Q2: 如何实现更细粒度的权限控制？](#q2-如何实现更细粒度的权限控制) `:1372:1400`
  - [Q3: 如何让用户可以修改自己的资料但不能修改他人？](#q3-如何让用户可以修改自己的资料但不能修改他人) `:1401:1417`
  - [Q4: 如何处理跨域请求？](#q4-如何处理跨域请求) `:1418:1435`
  - [Q5: Token 被盗用怎么办？](#q5-token-被盗用怎么办) `:1436:1458`
  - [Q6: 如何添加新的权限？](#q6-如何添加新的权限) `:1459:1491`
- [技术架构总结](#技术架构总结) `:1492:1493`
  - [优势](#优势) `:1494:1501`
  - [限制](#限制) `:1502:1507`
  - [扩展方向](#扩展方向) `:1508:1515`
- [参考资料](#参考资料) `:1516:1522`
