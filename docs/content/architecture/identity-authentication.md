...
- [功能特性](#功能特性) `:52+10`
- [快速开始](#快速开始) `:62+66`
  - [1. 确保服务运行](#1-确保服务运行) `:64+10`
  - [2. 注册用户](#2-注册用户) `:74+34`
  - [3. 用户登录](#3-用户登录) `:108+13`
  - [4. 使用 Token 访问受保护端点](#4-使用-token-访问受保护端点) `:121+7`
- [架构设计](#架构设计) `:128+108`
  - [代码结构](#代码结构) `:130+26`
  - [分层实现](#分层实现) `:156+69`
  - [JWT Claims 结构](#jwt-claims-结构) `:225+11`
- [工作流程](#工作流程) `:236+68`
  - [注册流程](#注册流程) `:238+16`
  - [登录流程](#登录流程) `:254+16`
  - [Token 验证流程](#token-验证流程) `:270+16`
  - [Token 刷新流程](#token-刷新流程) `:286+18`
- [配置说明](#配置说明) `:304+30`
  - [配置文件](#配置文件) `:306+11`
  - [环境变量](#环境变量) `:317+17`
- [API 端点](#api-端点) `:334+22`
  - [公开端点 (无需认证)](#公开端点-无需认证) `:336+8`
  - [受保护端点 (需要认证)](#受保护端点-需要认证) `:344+12`
- [安全特性](#安全特性) `:356+35`
- [使用示例](#使用示例) `:391+67`
  - [完整的认证流程](#完整的认证流程) `:393+41`
  - [在代码中使用认证服务](#在代码中使用认证服务) `:434+24`
- [JWT 最佳实践](#jwt-最佳实践) `:458+82`
  - [1. Token 存储](#1-token-存储) `:460+12`
  - [2. Token 刷新策略](#2-token-刷新策略) `:472+37`
  - [3. 安全建议](#3-安全建议) `:509+9`
  - [4. Token 黑名单 (可选)](#4-token-黑名单-可选) `:518+22`
- [扩展功能建议](#扩展功能建议) `:540+60`
  - [1. 邮箱验证](#1-邮箱验证) `:542+18`
  - [2. 密码重置](#2-密码重置) `:560+16`
  - [3. 双因素认证 (2FA)](#3-双因素认证-2fa) `:576+15`
  - [4. OAuth 社交登录](#4-oauth-社交登录) `:591+9`
- [故障排查](#故障排查) `:600+24`
  - [Token 验证失败](#token-验证失败) `:602+10`
  - [密码验证失败](#密码验证失败) `:612+6`
  - [用户无法登录](#用户无法登录) `:618+6`
- [下一步](#下一步) `:624+5`
