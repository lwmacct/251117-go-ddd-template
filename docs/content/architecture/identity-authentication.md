...
- [功能特性](#功能特性) `:50:59`
- [快速开始](#快速开始) `:60:61`
  - [1. 确保服务运行](#1-确保服务运行) `:62:71`
  - [2. 注册用户](#2-注册用户) `:72:105`
  - [3. 用户登录](#3-用户登录) `:106:118`
  - [4. 使用 Token 访问受保护端点](#4-使用-token-访问受保护端点) `:119:125`
- [架构设计](#架构设计) `:126:127`
  - [代码结构](#代码结构) `:128:153`
  - [分层实现](#分层实现) `:154:222`
  - [JWT Claims 结构](#jwt-claims-结构) `:223:233`
- [工作流程](#工作流程) `:234:235`
  - [注册流程](#注册流程) `:236:251`
  - [登录流程](#登录流程) `:252:267`
  - [Token 验证流程](#token-验证流程) `:268:283`
  - [Token 刷新流程](#token-刷新流程) `:284:301`
- [配置说明](#配置说明) `:302:303`
  - [配置文件](#配置文件) `:304:314`
  - [环境变量](#环境变量) `:315:331`
- [API 端点](#api-端点) `:332:333`
  - [公开端点 (无需认证)](#公开端点-无需认证) `:334:341`
  - [受保护端点 (需要认证)](#受保护端点-需要认证) `:342:353`
- [安全特性](#安全特性) `:354:388`
- [使用示例](#使用示例) `:389:390`
  - [完整的认证流程](#完整的认证流程) `:391:431`
  - [在代码中使用认证服务](#在代码中使用认证服务) `:432:455`
- [JWT 最佳实践](#jwt-最佳实践) `:456:457`
  - [1. Token 存储](#1-token-存储) `:458:469`
  - [2. Token 刷新策略](#2-token-刷新策略) `:470:506`
  - [3. 安全建议](#3-安全建议) `:507:515`
  - [4. Token 黑名单 (可选)](#4-token-黑名单-可选) `:516:537`
- [扩展功能建议](#扩展功能建议) `:538:539`
  - [1. 邮箱验证](#1-邮箱验证) `:540:557`
  - [2. 密码重置](#2-密码重置) `:558:573`
  - [3. 双因素认证 (2FA)](#3-双因素认证-2fa) `:574:588`
  - [4. OAuth 社交登录](#4-oauth-社交登录) `:589:597`
- [故障排查](#故障排查) `:598:599`
  - [Token 验证失败](#token-验证失败) `:600:609`
  - [密码验证失败](#密码验证失败) `:610:615`
  - [用户无法登录](#用户无法登录) `:616:621`
- [下一步](#下一步) `:622:626`
