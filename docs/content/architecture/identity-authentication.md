...
- [功能特性](#功能特性) `:50+10`
- [快速开始](#快速开始) `:60+66`
  - [1. 确保服务运行](#1-确保服务运行) `:62+10`
  - [2. 注册用户](#2-注册用户) `:72+34`
  - [3. 用户登录](#3-用户登录) `:106+13`
  - [4. 使用 Token 访问受保护端点](#4-使用-token-访问受保护端点) `:119+7`
- [架构设计](#架构设计) `:126+108`
  - [代码结构](#代码结构) `:128+26`
  - [分层实现](#分层实现) `:154+69`
  - [JWT Claims 结构](#jwt-claims-结构) `:223+11`
- [工作流程](#工作流程) `:234+68`
  - [注册流程](#注册流程) `:236+16`
  - [登录流程](#登录流程) `:252+16`
  - [Token 验证流程](#token-验证流程) `:268+16`
  - [Token 刷新流程](#token-刷新流程) `:284+18`
- [配置说明](#配置说明) `:302+30`
  - [配置文件](#配置文件) `:304+11`
  - [环境变量](#环境变量) `:315+17`
- [API 端点](#api-端点) `:332+22`
  - [公开端点 (无需认证)](#公开端点-无需认证) `:334+8`
  - [受保护端点 (需要认证)](#受保护端点-需要认证) `:342+12`
- [安全特性](#安全特性) `:354+35`
- [使用示例](#使用示例) `:389+67`
  - [完整的认证流程](#完整的认证流程) `:391+41`
  - [在代码中使用认证服务](#在代码中使用认证服务) `:432+24`
- [JWT 最佳实践](#jwt-最佳实践) `:456+82`
  - [1. Token 存储](#1-token-存储) `:458+12`
  - [2. Token 刷新策略](#2-token-刷新策略) `:470+37`
  - [3. 安全建议](#3-安全建议) `:507+9`
  - [4. Token 黑名单 (可选)](#4-token-黑名单-可选) `:516+22`
- [扩展功能建议](#扩展功能建议) `:538+60`
  - [1. 邮箱验证](#1-邮箱验证) `:540+18`
  - [2. 密码重置](#2-密码重置) `:558+16`
  - [3. 双因素认证 (2FA)](#3-双因素认证-2fa) `:574+15`
  - [4. OAuth 社交登录](#4-oauth-社交登录) `:589+9`
- [故障排查](#故障排查) `:598+24`
  - [Token 验证失败](#token-验证失败) `:600+10`
  - [密码验证失败](#密码验证失败) `:610+6`
  - [用户无法登录](#用户无法登录) `:616+6`
- [下一步](#下一步) `:622+5`
