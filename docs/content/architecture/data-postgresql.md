...
- [功能特性](#功能特性) `:48:58`
- [快速开始](#快速开始) `:59:60`
  - [1. 启动 PostgreSQL](#1-启动-postgresql) `:61:79`
  - [2. 配置数据库连接](#2-配置数据库连接) `:80:95`
  - [3. 运行应用](#3-运行应用) `:96:107`
- [架构设计](#架构设计) `:108:109`
  - [代码结构](#代码结构) `:110:132`
  - [用户领域模型](#用户领域模型) `:133:155`
  - [持久化模型（Infrastructure）](#持久化模型infrastructure) `:156:182`
  - [仓储接口（CQRS）](#仓储接口cqrs) `:183:215`
- [连接池配置](#连接池配置) `:216:231`
- [API 端点](#api-端点) `:232:233`
  - [健康检查](#健康检查) `:234:255`
  - [用户 CRUD (需要认证)](#用户-crud-需要认证) `:256:305`
- [使用示例](#使用示例) `:306:307`
  - [在代码中使用仓储](#在代码中使用仓储) `:308:350`
- [特性说明](#特性说明) `:351:352`
  - [1. 自动迁移](#1-自动迁移) `:353:371`
  - [2. 软删除](#2-软删除) `:372:389`
  - [3. 密码加密](#3-密码加密) `:390:417`
  - [4. 数据验证](#4-数据验证) `:418:430`
- [连接字符串格式](#连接字符串格式) `:431:452`
- [性能优化](#性能优化) `:453:454`
  - [1. 索引优化](#1-索引优化) `:455:466`
  - [2. 批量操作](#2-批量操作) `:467:479`
  - [3. 预加载关联](#3-预加载关联) `:480:487`
  - [4. 选择字段](#4-选择字段) `:488:497`
- [事务支持](#事务支持) `:498:525`
- [故障排查](#故障排查) `:526:527`
  - [连接失败](#连接失败) `:528:543`
  - [迁移失败](#迁移失败) `:544:563`
...
- [最佳实践](#最佳实践) `:588:597`
- [扩展功能](#扩展功能) `:598:599`
  - [1. 添加新的领域模型](#1-添加新的领域模型) `:600:627`
  - [2. 全文搜索](#2-全文搜索) `:628:635`
  - [3. 读写分离](#3-读写分离) `:636:647`
- [下一步](#下一步) `:648:653`
