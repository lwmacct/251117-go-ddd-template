...
- [功能特性](#功能特性) `:50+11`
- [快速开始](#快速开始) `:61+49`
  - [1. 启动 PostgreSQL](#1-启动-postgresql) `:63+19`
  - [2. 配置数据库连接](#2-配置数据库连接) `:82+16`
  - [3. 运行应用](#3-运行应用) `:98+12`
- [架构设计](#架构设计) `:110+108`
  - [代码结构](#代码结构) `:112+23`
  - [用户领域模型](#用户领域模型) `:135+23`
  - [持久化模型（Infrastructure）](#持久化模型infrastructure) `:158+27`
  - [仓储接口（CQRS）](#仓储接口cqrs) `:185+33`
- [连接池配置](#连接池配置) `:218+16`
- [API 端点](#api-端点) `:234+74`
  - [健康检查](#健康检查) `:236+22`
  - [用户 CRUD (需要认证)](#用户-crud-需要认证) `:258+50`
- [使用示例](#使用示例) `:308+45`
  - [在代码中使用仓储](#在代码中使用仓储) `:310+43`
- [特性说明](#特性说明) `:353+80`
  - [1. 自动迁移](#1-自动迁移) `:355+19`
  - [2. 软删除](#2-软删除) `:374+18`
  - [3. 密码加密](#3-密码加密) `:392+28`
  - [4. 数据验证](#4-数据验证) `:420+13`
- [连接字符串格式](#连接字符串格式) `:433+22`
- [性能优化](#性能优化) `:455+45`
  - [1. 索引优化](#1-索引优化) `:457+12`
  - [2. 批量操作](#2-批量操作) `:469+13`
  - [3. 预加载关联](#3-预加载关联) `:482+8`
  - [4. 选择字段](#4-选择字段) `:490+10`
- [事务支持](#事务支持) `:500+28`
- [故障排查](#故障排查) `:528+62`
  - [连接失败](#连接失败) `:530+16`
  - [迁移失败](#迁移失败) `:546+20`
...
- [最佳实践](#最佳实践) `:590+10`
- [扩展功能](#扩展功能) `:600+50`
  - [1. 添加新的领域模型](#1-添加新的领域模型) `:602+28`
  - [2. 全文搜索](#2-全文搜索) `:630+8`
  - [3. 读写分离](#3-读写分离) `:638+12`
- [下一步](#下一步) `:650+6`
