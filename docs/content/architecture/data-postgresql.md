...
- [功能特性](#功能特性) `:48+11`
- [快速开始](#快速开始) `:59+49`
  - [1. 启动 PostgreSQL](#1-启动-postgresql) `:61+19`
  - [2. 配置数据库连接](#2-配置数据库连接) `:80+16`
  - [3. 运行应用](#3-运行应用) `:96+12`
- [架构设计](#架构设计) `:108+108`
  - [代码结构](#代码结构) `:110+23`
  - [用户领域模型](#用户领域模型) `:133+23`
  - [持久化模型（Infrastructure）](#持久化模型infrastructure) `:156+27`
  - [仓储接口（CQRS）](#仓储接口cqrs) `:183+33`
- [连接池配置](#连接池配置) `:216+16`
- [API 端点](#api-端点) `:232+74`
  - [健康检查](#健康检查) `:234+22`
  - [用户 CRUD (需要认证)](#用户-crud-需要认证) `:256+50`
- [使用示例](#使用示例) `:306+45`
  - [在代码中使用仓储](#在代码中使用仓储) `:308+43`
- [特性说明](#特性说明) `:351+80`
  - [1. 自动迁移](#1-自动迁移) `:353+19`
  - [2. 软删除](#2-软删除) `:372+18`
  - [3. 密码加密](#3-密码加密) `:390+28`
  - [4. 数据验证](#4-数据验证) `:418+13`
- [连接字符串格式](#连接字符串格式) `:431+22`
- [性能优化](#性能优化) `:453+45`
  - [1. 索引优化](#1-索引优化) `:455+12`
  - [2. 批量操作](#2-批量操作) `:467+13`
  - [3. 预加载关联](#3-预加载关联) `:480+8`
  - [4. 选择字段](#4-选择字段) `:488+10`
- [事务支持](#事务支持) `:498+28`
- [故障排查](#故障排查) `:526+62`
  - [连接失败](#连接失败) `:528+16`
  - [迁移失败](#迁移失败) `:544+20`
...
- [最佳实践](#最佳实践) `:588+10`
- [扩展功能](#扩展功能) `:598+50`
  - [1. 添加新的领域模型](#1-添加新的领域模型) `:600+28`
  - [2. 全文搜索](#2-全文搜索) `:628+8`
  - [3. 读写分离](#3-读写分离) `:636+12`
- [下一步](#下一步) `:648+6`
