...
- [迁移阶段](#迁移阶段) `:67+791`
  ...
  - [阶段 7：更新依赖注入容器](#阶段-7更新依赖注入容器) `:541+118`
  - [阶段 8：完成核心模块 Application 层](#阶段-8完成核心模块-application-层) `:659+170`
  - [阶段 9：编译验证](#阶段-9编译验证) `:829+29`
- [完成模块清单](#完成模块清单) `:858+195`
  - [核心业务模块 (Application 层 100% 完成)](#核心业务模块-application-层-100-完成) `:860+163`
  - [已有模块 (Application 层已完成)](#已有模块-application-层已完成) `:1023+16`
  - [基础设施模块 (无需 Application 层)](#基础设施模块-无需-application-层) `:1039+14`
- [成果对比](#成果对比) `:1053+14`
- [最佳实践](#最佳实践) `:1067+22`
  - [Use Case 命名规范](#use-case-命名规范) `:1069+6`
  - [依赖注入原则](#依赖注入原则) `:1075+6`
  - [CQRS 适用场景](#cqrs-适用场景) `:1081+8`
- [后续优化建议](#后续优化建议) `:1089+85`
  - [1. 性能优化](#1-性能优化) `:1091+31`
  - [2. 搜索优化](#2-搜索优化) `:1122+15`
  - [3. 测试覆盖](#3-测试覆盖) `:1137+37`
- [迁移验证清单](#迁移验证清单) `:1174+49`
  - [每个模块迁移完成后检查](#每个模块迁移完成后检查) `:1176+47`
- [常见问题](#常见问题) `:1223+143`
  - [Q1: 所有模块是否都已完成迁移？](#q1-所有模块是否都已完成迁移) `:1225+25`
  - [Q2: Container 新旧代码已清理完成吗？](#q2-container-新旧代码已清理完成吗) `:1250+27`
  - [Q3: 如何处理现有的 Service？](#q3-如何处理现有的-service) `:1277+23`
  - [Q4: CQRS 是否所有模块都必须？](#q4-cqrs-是否所有模块都必须) `:1300+27`
  - [Q5: 如何为新功能添加 Use Case？](#q5-如何为新功能添加-use-case) `:1327+39`
- [相关文档](#相关文档) `:1366+10`
