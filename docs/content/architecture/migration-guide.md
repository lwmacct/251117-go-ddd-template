...
- [重构概览](#重构概览) `:45+20`
  - [解决的核心问题](#解决的核心问题) `:47+7`
  - [迁移成果](#迁移成果) `:54+11`
- [迁移阶段](#迁移阶段) `:65+791`
  - [阶段 1：创建 Application 层结构](#阶段-1创建-application-层结构) `:67+30`
  - [阶段 2：重构 Domain 层](#阶段-2重构-domain-层) `:97+96`
  - [阶段 3：实现 CQRS Repository](#阶段-3实现-cqrs-repository) `:193+80`
  - [阶段 4：创建 Application Use Cases](#阶段-4创建-application-use-cases) `:273+135`
  - [阶段 5：重构 Infrastructure Service](#阶段-5重构-infrastructure-service) `:408+56`
  - [阶段 6：更新 Adapter 层](#阶段-6更新-adapter-层) `:464+75`
  - [阶段 7：更新依赖注入容器](#阶段-7更新依赖注入容器) `:539+118`
  - [阶段 8：完成核心模块 Application 层](#阶段-8完成核心模块-application-层) `:657+170`
  - [阶段 9：编译验证](#阶段-9编译验证) `:827+29`
- [完成模块清单](#完成模块清单) `:856+195`
  - [核心业务模块 (Application 层 100% 完成)](#核心业务模块-application-层-100-完成) `:858+163`
  - [已有模块 (Application 层已完成)](#已有模块-application-层已完成) `:1021+16`
  - [基础设施模块 (无需 Application 层)](#基础设施模块-无需-application-层) `:1037+14`
- [成果对比](#成果对比) `:1051+14`
- [最佳实践](#最佳实践) `:1065+22`
  - [Use Case 命名规范](#use-case-命名规范) `:1067+6`
  - [依赖注入原则](#依赖注入原则) `:1073+6`
  - [CQRS 适用场景](#cqrs-适用场景) `:1079+8`
- [后续优化建议](#后续优化建议) `:1087+85`
  - [1. 性能优化](#1-性能优化) `:1089+31`
  - [2. 搜索优化](#2-搜索优化) `:1120+15`
  - [3. 测试覆盖](#3-测试覆盖) `:1135+37`
- [迁移验证清单](#迁移验证清单) `:1172+49`
  - [每个模块迁移完成后检查](#每个模块迁移完成后检查) `:1174+47`
- [常见问题](#常见问题) `:1221+143`
  - [Q1: 所有模块是否都已完成迁移？](#q1-所有模块是否都已完成迁移) `:1223+25`
  - [Q2: Container 新旧代码已清理完成吗？](#q2-container-新旧代码已清理完成吗) `:1248+27`
  - [Q3: 如何处理现有的 Service？](#q3-如何处理现有的-service) `:1275+23`
  - [Q4: CQRS 是否所有模块都必须？](#q4-cqrs-是否所有模块都必须) `:1298+27`
  - [Q5: 如何为新功能添加 Use Case？](#q5-如何为新功能添加-use-case) `:1325+39`
- [相关文档](#相关文档) `:1364+10`
