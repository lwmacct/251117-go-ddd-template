...
- [架构概览](#架构概览) `:37+188`
  - [四层架构](#四层架构) `:39+16`
  - [各层职责](#各层职责) `:55+170`
- [CQRS 模式详解](#cqrs-模式详解) `:225+86`
  - [Command（命令 - 写操作）](#command命令-写操作) `:227+59`
  - [Query（查询 - 读操作）](#query查询-读操作) `:286+25`
- [依赖注入流程](#依赖注入流程) `:311+38`
  - [Container 注册顺序](#container-注册顺序) `:313+36`
- [真实实现案例](#真实实现案例) `:349+282`
  - [案例 1: PAT Token 创建 (安全设计模式)](#案例-1-pat-token-创建-安全设计模式) `:351+72`
  - [案例 2: AuditLog 复杂查询 (多维度过滤)](#案例-2-auditlog-复杂查询-多维度过滤) `:423+58`
  - [案例 3: Setting 批量更新 (事务处理)](#案例-3-setting-批量更新-事务处理) `:481+86`
  - [案例 4: Menu 树形结构排序 (复杂业务逻辑)](#案例-4-menu-树形结构排序-复杂业务逻辑) `:567+64`
- [开发指南](#开发指南) `:631+91`
  - [如何添加新功能](#如何添加新功能) `:633+89`
- [架构对比](#架构对比) `:722+14`
- [优势](#优势) `:736+54`
  - [1. 职责分离清晰](#1-职责分离清晰) `:738+7`
  - [2. 可测试性强](#2-可测试性强) `:745+22`
  - [3. 易于扩展](#3-易于扩展) `:767+6`
  - [4. 业务意图明确](#4-业务意图明确) `:773+17`
- [当前实现状态](#当前实现状态) `:790+50`
  - [完成模块清单 (2025-11-19)](#完成模块清单-2025-11-19) `:792+14`
  - [统计数据](#统计数据) `:806+22`
  - [架构完整性](#架构完整性) `:828+12`
- [相关资源](#相关资源) `:840+19`
  - [推荐阅读顺序](#推荐阅读顺序) `:846+13`
