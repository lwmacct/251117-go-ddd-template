...
- [架构概览](#架构概览) `:37:38`
  - [四层架构](#四层架构) `:39:54`
  - [各层职责](#各层职责) `:55:56`
- [CQRS 模式详解](#cqrs-模式详解) `:225:226`
  - [Command（命令 - 写操作）](#command命令-写操作) `:227:285`
  - [Query（查询 - 读操作）](#query查询-读操作) `:286:310`
- [依赖注入流程](#依赖注入流程) `:311:312`
  - [Container 注册顺序](#container-注册顺序) `:313:348`
- [真实实现案例](#真实实现案例) `:349:350`
  - [案例 1: PAT Token 创建 (安全设计模式)](#案例-1-pat-token-创建-安全设计模式) `:351:422`
  - [案例 2: AuditLog 复杂查询 (多维度过滤)](#案例-2-auditlog-复杂查询-多维度过滤) `:423:480`
  - [案例 3: Setting 批量更新 (事务处理)](#案例-3-setting-批量更新-事务处理) `:481:566`
  - [案例 4: Menu 树形结构排序 (复杂业务逻辑)](#案例-4-menu-树形结构排序-复杂业务逻辑) `:567:630`
- [开发指南](#开发指南) `:631:632`
  - [如何添加新功能](#如何添加新功能) `:633:634`
- [架构对比](#架构对比) `:722:735`
- [优势](#优势) `:736:737`
  - [1. 职责分离清晰](#1-职责分离清晰) `:738:744`
  - [2. 可测试性强](#2-可测试性强) `:745:766`
  - [3. 易于扩展](#3-易于扩展) `:767:772`
  - [4. 业务意图明确](#4-业务意图明确) `:773:789`
- [当前实现状态](#当前实现状态) `:790:791`
  - [完成模块清单 (2025-11-19)](#完成模块清单-2025-11-19) `:792:805`
  - [统计数据](#统计数据) `:806:827`
  - [架构完整性](#架构完整性) `:828:839`
- [相关资源](#相关资源) `:840:845`
  - [推荐阅读顺序](#推荐阅读顺序) `:846:858`
