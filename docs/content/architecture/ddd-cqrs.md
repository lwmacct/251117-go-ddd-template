...
- [架构概览](#架构概览) `:42+188`
  - [四层架构](#四层架构) `:44+16`
  - [各层职责](#各层职责) `:60+170`
- [CQRS 模式详解](#cqrs-模式详解) `:230+86`
  - [Command（命令 - 写操作）](#command命令-写操作) `:232+59`
  - [Query（查询 - 读操作）](#query查询-读操作) `:291+25`
- [依赖注入流程](#依赖注入流程) `:316+95`
  - [Container 模块化架构](#container-模块化架构) `:318+16`
  - [初始化顺序（每步依赖前步）](#初始化顺序每步依赖前步) `:334+31`
  - [模块访问示例](#模块访问示例) `:365+28`
  - [文件结构](#文件结构) `:393+18`
- [真实实现案例](#真实实现案例) `:411+282`
  - [案例 1: PAT Token 创建 (安全设计模式)](#案例-1-pat-token-创建-安全设计模式) `:413+72`
  - [案例 2: AuditLog 复杂查询 (多维度过滤)](#案例-2-auditlog-复杂查询-多维度过滤) `:485+58`
  - [案例 3: Setting 批量更新 (事务处理)](#案例-3-setting-批量更新-事务处理) `:543+86`
  - [案例 4: Menu 树形结构排序 (复杂业务逻辑)](#案例-4-menu-树形结构排序-复杂业务逻辑) `:629+64`
- [开发指南](#开发指南) `:693+91`
  - [如何添加新功能](#如何添加新功能) `:695+89`
- [架构对比](#架构对比) `:784+14`
- [优势](#优势) `:798+54`
  - [1. 职责分离清晰](#1-职责分离清晰) `:800+7`
  - [2. 可测试性强](#2-可测试性强) `:807+22`
  - [3. 易于扩展](#3-易于扩展) `:829+6`
  - [4. 业务意图明确](#4-业务意图明确) `:835+17`
- [当前实现状态](#当前实现状态) `:852+50`
  - [完成模块清单 (2025-11-19)](#完成模块清单-2025-11-19) `:854+14`
  - [统计数据](#统计数据) `:868+22`
  - [架构完整性](#架构完整性) `:890+12`
- [相关资源](#相关资源) `:902+19`
  - [推荐阅读顺序](#推荐阅读顺序) `:908+13`
...
Container 采用模块化设计，将依赖按职责聚合为 6 个功能模块：

```go
type Container struct {
    Config   *config.Config
    Infra    *InfrastructureModule  // DB, Redis, EventBus
    Repos    *RepositoriesModule    // 所有 CQRS 仓储
...
// 访问基础设施
container.Infra.DB
container.Infra.RedisClient
container.Infra.EventBus

// 访问仓储
container.Repos.User.Query       // User 读仓储
container.Repos.User.Command     // User 写仓储
container.Repos.Role.Query       // Role 读仓储

// 访问服务
container.Services.Auth          // 认证领域服务
container.Services.JWT           // JWT 管理器
container.Services.PermissionCache // 权限缓存服务

// 访问用例
container.UseCases.User.Create   // 创建用户用例
container.UseCases.User.Get      // 获取用户用例
