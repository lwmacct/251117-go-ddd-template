...
- [功能特性](#功能特性) `:49:59`
- [快速开始](#快速开始) `:60:61`
  - [1. 启动 Redis 服务](#1-启动-redis-服务) `:62:79`
  - [2. 配置 Redis 连接](#2-配置-redis-连接) `:80:109`
  - [3. 运行应用](#3-运行应用) `:110:119`
- [架构设计](#架构设计) `:120:121`
  - [代码结构](#代码结构) `:122:138`
  - [CacheRepository 接口](#cacherepository-接口) `:139:159`
- [API 端点](#api-端点) `:160:161`
  - [健康检查](#健康检查) `:162:177`
  - [设置缓存](#设置缓存) `:178:189`
  - [获取缓存](#获取缓存) `:190:207`
  - [删除缓存](#删除缓存) `:208:213`
- [使用示例](#使用示例) `:214:215`
  ...
  - [缓存结构体](#缓存结构体) `:255:272`
  - [分布式锁](#分布式锁) `:273:290`
  - [缓存更新模式](#缓存更新模式) `:291:292`
- [常用场景](#常用场景) `:351:352`
  - [1. 会话管理](#1-会话管理) `:353:372`
  - [2. API 限流](#2-api-限流) `:373:404`
  - [3. 防止缓存穿透](#3-防止缓存穿透) `:405:436`
  - [4. 防止缓存雪崩](#4-防止缓存雪崩) `:437:463`
  - [5. 排行榜 (Sorted Set)](#5-排行榜-sorted-set) `:464:483`
- [性能优化](#性能优化) `:484:485`
  - [1. Pipeline (批量操作)](#1-pipeline-批量操作) `:486:499`
  - [2. 连接池配置](#2-连接池配置) `:500:512`
- [故障排查](#故障排查) `:513:514`
  - [连接失败](#连接失败) `:515:533`
  - [性能问题](#性能问题) `:534:549`
  - [键过期问题](#键过期问题) `:550:562`
- [最佳实践](#最佳实践) `:563:589`
- [扩展功能](#扩展功能) `:590:591`
  - [1. Redis Cluster 支持](#1-redis-cluster-支持) `:592:605`
  - [2. Redis 哨兵模式](#2-redis-哨兵模式) `:606:614`
  - [3. 发布/订阅](#3-发布订阅) `:615:631`
  - [4. Lua 脚本](#4-lua-脚本) `:632:647`
- [注意事项](#注意事项) `:648:670`
- [下一步](#下一步) `:671:676`
