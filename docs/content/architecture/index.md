...
- [文档列表](#文档列表) `:55:56`
  - [核心架构](#核心架构) `:57:62`
  - [数据与基础设施](#数据与基础设施) `:63:67`
  - [身份与访问控制](#身份与访问控制) `:68:74`
- [文档定位](#文档定位) `:75:76`
  - [目标读者](#目标读者) `:77:82`
  - [内容特点](#内容特点) `:83:90`
- [快速导航](#快速导航) `:91:92`
...
- [相关文档](#相关文档) `:122:127`
- [架构亮点](#架构亮点) `:128:129`
  - [1. 三段式细粒度权限](#1-三段式细粒度权限) `:130:145`
  - [2. 双重认证机制](#2-双重认证机制) `:146:158`
  - [3. DDD + CQRS 四层架构](#3-ddd-cqrs-四层架构) `:159:184`
  - [4. 审计日志系统](#4-审计日志系统) `:185:200`
- [技术栈](#技术栈) `:201:202`
  - [后端核心](#后端核心) `:203:211`
  - [数据存储](#数据存储) `:212:216`
- [架构图](#架构图) `:217:218`
...
3. [架构分层与目录](./architecture-layers.md) - 统一分层与依赖关系
   :::
...
- **[DDD + CQRS 架构详解](./ddd-cqrs.md)** ⭐ **推荐** - 当前使用的四层架构 + CQRS 模式
- **[架构分层与目录](./architecture-layers.md)** - 统一架构图、目录说明与依赖关系
- **[架构迁移指南](./migration-guide.md)** - 重构过程、最佳实践、后续优化

### 数据与基础设施

- **[PostgreSQL 架构](./data-postgresql.md)** - 数据模型设计、GORM ORM、迁移策略
- **[Redis 架构](./data-redis.md)** - 缓存策略、分布式锁、会话存储
...
1. 从 [架构分层与目录](./architecture-layers.md) 开始
2. 深入阅读 [DDD + CQRS 架构详解](./ddd-cqrs.md)
3. 理解迁移演进过程 [架构迁移指南](./migration-guide.md)

#### 数据库是如何设计的？

1. 阅读 [PostgreSQL 架构](./data-postgresql.md)
2. 了解领域模型和数据表映射
3. 学习 GORM 的使用技巧
4. 查看迁移和种子数据策略
