...
- [相关文档](#相关文档) `:114+6`
- [架构亮点](#架构亮点) `:120+76`
  - [1. URN 风格细粒度权限](#1-urn-风格细粒度权限) `:122+19`
  - [2. 双重认证机制](#2-双重认证机制) `:141+13`
  - [3. DDD + CQRS 四层架构](#3-ddd-cqrs-四层架构) `:154+26`
  - [4. 审计日志系统](#4-审计日志系统) `:180+16`
- [技术栈](#技术栈) `:196+16`
  - [后端核心](#后端核心) `:198+9`
  - [数据存储](#数据存储) `:207+5`
- [架构图](#架构图) `:212+67`
...
- **[URN 风格 RBAC](./rbac-urn.md)** - 统一资源名称格式的权限系统
...
{scope}:{type}:{identifier}
   ↓       ↓        ↓
sys:users:create    # 系统级-用户模块-创建操作
self:profile:update # 用户自身-资料-更新操作
public:auth:login   # 公开-认证-登录操作
```

**特性**:

- 统一 Operation 和 Resource 格式
- 支持通配符匹配（`sys:users:*`、`*:*:*`）
- Scope 层级通配（`sys.*:*:*` 匹配子域）
- 运行时变量替换（`@me`、`@org`）

详见 [URN 风格 RBAC](./rbac-urn.md)
