...
- [目标分析](#目标分析) `:55+20`
  - [当前状态](#当前状态) `:57+11`
  - [待完成后端功能](#待完成后端功能) `:68+7`
- [阶段 1: 实现批量用户导入 API](#阶段-1-实现批量用户导入-api) `:75+67`
  - [目标](#目标) `:77+4`
  - [API 设计](#api-设计) `:81+35`
  - [实现步骤](#实现步骤) `:116+9`
  - [涉及文件](#涉及文件) `:125+17`
- [阶段 2: 补充 Infrastructure 层测试](#阶段-2-补充-infrastructure-层测试) `:142+57`
  - [目标](#目标-1) `:144+4`
  - [测试策略](#测试策略) `:148+4`
  - [待测试模块](#待测试模块) `:152+12`
  - [实现步骤](#实现步骤-1) `:164+10`
  - [测试辅助代码示例](#测试辅助代码示例) `:174+25`
- [阶段 3: 提升整体测试覆盖率](#阶段-3-提升整体测试覆盖率) `:199+28`
  - [目标](#目标-2) `:201+4`
  - [优先级排序](#优先级排序) `:205+13`
  - [实现步骤](#实现步骤-2) `:218+9`
- [阶段 4: 添加集成测试框架](#阶段-4-添加集成测试框架) `:227+34`
  - [目标](#目标-3) `:229+4`
  - [测试范围](#测试范围) `:233+6`
  - [技术方案](#技术方案) `:239+11`
  - [实现步骤](#实现步骤-3) `:250+11`
- [建议执行顺序](#建议执行顺序) `:261+24`
  - [快速启动选项](#快速启动选项) `:275+10`
- [注意事项](#注意事项) `:285+9`
- [完成成果](#完成成果) `:294+33`
  - [已完成目标](#已完成目标) `:296+8`
  - [新增文件](#新增文件) `:304+16`
