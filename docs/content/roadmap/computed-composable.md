...
const { state: user, isLoading, error, execute } = computedAsync(
  async () => {
...
const { state, pause, resume, trigger, isPaused } = computedWithControl(
  () => count.value * 2
)
...
const { current, history, canUndo, undo, clear } = computedWithHistory(
  () => count.value * 2,
  { capacity: 10 }
)
