...
import type { LoginRequest, RegisterRequest, CaptchaData, AuthTokens } from "@/types/auth";
...
export class AuthAPI {
   ...
   * 登录（带验证码）
   */
  static async login(req: LoginRequest): Promise<ApiResponse<AuthTokens>> {
    const { data } = await apiClient.post<ApiResponse<AuthTokens>>("/api/auth/login", req);
    return data;
  }
  ...
  static async verifyEmail(params: {
    ...
    const { data } = await apiClient.post<ApiResponse<AuthTokens>>("/api/auth/verify-email", params);
    return data;
  }
   ...
   */
  static async verify2FA(params: { session_token: string; code: string }): Promise<ApiResponse<AuthTokens>> {
    const { data } = await apiClient.post<ApiResponse<AuthTokens>>("/api/auth/login", {
