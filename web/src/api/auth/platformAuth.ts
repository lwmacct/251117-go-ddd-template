...
export class PlatformAuthAPI {
  ...
  static async getCaptcha(): Promise<PlatformApiResponse<CaptchaData>> {
    try {
      const { data } = await apiClient.get<PlatformApiResponse<CaptchaData>>(
        "/auth/captcha"
      );
      return data;
    } catch (error: any) {
  ...
  static async login(
    ...
    try {
      const { data } = await apiClient.post<PlatformApiResponse<any>>(
        "/auth/login",
        req
      );
      return data;
  ...
  static async register(
    ...
    PlatformApiResponse<{ session_token?: string; message?: string }>
  > {
    try {
