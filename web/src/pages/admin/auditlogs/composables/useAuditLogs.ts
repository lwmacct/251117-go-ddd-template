  ...
  const exportLogs = async () => {
    ...
    try {
      ...
      const columns: CSVColumn<AuditLog>[] = [
        ...
        { header: "用户名", key: (item) => item.username || "" },
        { header: "操作类型", key: "action" },
        { header: "资源类型", key: "resource" },
        { header: "资源ID", key: (item) => item.resource_id?.toString() || "" },
        { header: "状态", key: "status" },
        { header: "IP地址", key: (item) => item.ip_address || "" },
        { header: "User Agent", key: (item) => item.user_agent || "" },
      ...
      // 导出 CSV
      exportToCSV(response.data, columns, { filename });

      successMessage.value = `成功导出 ${response.data.length} 条记录`;
    } catch (error) {
