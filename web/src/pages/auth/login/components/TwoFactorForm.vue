...
<template>
  <div class="two-factor-form-wrapper">
    <v-card class="two-factor-card" elevation="12">
      <!-- 返回按钮 -->
      <div class="pa-4 pb-0">
        <v-btn variant="text" prepend-icon="mdi-arrow-left" size="small" @click="goBack">
          返回登录
        </v-btn>
      </div>
      ...
      <v-card-text class="px-8 pb-8">
        ...
        <v-form @submit.prevent="handleVerify">
          <v-text-field v-model="twoFactorCode" label="验证码" prepend-inner-icon="mdi-shield-check" variant="outlined"
            type="text" required :maxlength="20" :disabled="isLoading" placeholder="请输入6位验证码或恢复码"
            autocomplete="one-time-code" autofocus class="mb-4" :error="errorMessage.length > 0"
            :error-messages="errorMessage" hint="输入您手机应用中的6位验证码，或使用恢复码" persistent-hint>
            <template #append-inner>
              <v-btn icon="mdi-help-circle-outline" variant="text" size="small"
                @click="showRecoveryCodeHelp = !showRecoveryCodeHelp"></v-btn>
            </template>
          </v-text-field>
          ...
          <v-expand-transition>
            <v-alert v-if="showRecoveryCodeHelp" type="warning" variant="tonal" density="compact" class="mb-4">
              <div class="text-body-2">
                ...
                <div>
                  如果您无法访问认证器应用，可以使用恢复码。恢复码是在设置2FA时生成的备用验证码，请妥善保管。
                </div>
              </div>
            </v-alert>
          </v-expand-transition>

          <v-btn color="primary" variant="elevated" prepend-icon="mdi-check" block size="large" type="submit"
            :loading="isLoading" :disabled="!isCodeValid || isLoading" class="mt-2">
            {{ isLoading ? '验证中...' : '验证' }}
          </v-btn>
        </v-form>
      </v-card-text>
