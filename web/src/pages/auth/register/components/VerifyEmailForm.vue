    ...
    <v-card class="verify-email-card" elevation="12">
      ...
      <v-card-text class="px-8 pb-8">
        ...
        <template v-if="isVerified">
          ...
          <v-btn v-if="!registerStore.sessionToken.value" color="primary" variant="elevated" block
        ...
        <template v-else>
          <v-alert type="info" variant="tonal" density="compact" class="mb-6">
            <div class="text-body-2">
              ...
              <div class="text-medium-emphasis">{{ registerStore.email.value || '您的邮箱' }}</div>
              <div class="mt-2">请查收邮件并输入6位验证码完成注册验证。</div>
            </div>
          </v-alert>

          <v-form @submit.prevent="handleVerify">
            <v-text-field v-model="verificationCode" label="验证码" prepend-inner-icon="mdi-shield-check"
              variant="outlined" type="text" required :maxlength="6" :disabled="isLoading" placeholder="请输入6位验证码"
              autocomplete="one-time-code" autofocus class="mb-4" :error="errorMessage.length > 0"
              :error-messages="errorMessage" hint="请输入邮件中收到的6位数字验证码" persistent-hint></v-text-field>
