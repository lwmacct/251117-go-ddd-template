    ...
    <v-card>
      ...
      <v-card-text>
        <v-form ref="form" v-model="valid">
          <v-text-field v-model="formData.name" label="Token 名称" :rules="rules.name" variant="outlined" density="comfortable" class="mb-2" hint="给这个 Token 一个描述性的名称" persistent-hint></v-text-field>
          ...
          <div class="mt-4">
            ...
            <div v-if="expiresEnabled" class="ml-8 mt-2">
              <v-chip-group>
                <v-chip v-for="option in expirationOptions" :key="option.value" @click="setExpiration(option.value)" size="small">
                  {{ option.title }}
                </v-chip>
              </v-chip-group>

              <v-text-field v-model="formData.expires_at" label="过期时间" type="datetime-local" variant="outlined" density="compact" class="mt-2" hint="留空则永不过期"></v-text-field>
            </div>
          </div>
          ...
          <div class="mt-4">
            ...
            <div v-if="ipWhitelistEnabled" class="ml-8 mt-2">
              <v-textarea v-model="ipWhitelistText" label="IP 地址列表（每行一个）" variant="outlined" rows="3" hint="例如: 192.168.1.1" persistent-hint></v-textarea>
            </div>
          </div>
        </v-form>
      ...
      <v-card-actions>
        ...
        <v-btn color="primary" variant="elevated" @click="handleSave" :disabled="!valid">创建</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
